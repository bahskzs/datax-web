 ...
 */
@Component
@ConditionalOnProperty(prefix = "yth.history", name = "enable", havingValue = "true")
@Slf4j
public class CacheWithScheduledExecutor {
    ...
    public List<YTHUserDTO> getUsers(String areaCode, String code, String name) {
        ...
        for (Map.Entry<String, YTHUserDTO> entry : cache.entrySet()) {
            ...
            // log.info("key {}", key);
            res = filterData(Arrays.asList(key), name, code, areaCode);
            // res非空,那么当前key可用
            if (!res.isEmpty()) {
    ...
    private List<String> filterData(List<String> dataList, String name, String code, String area) {
    ...
    private void store(YTHResponse response) {
        ...
        for (YTHUserDTO dto : response.getData()) {
            String CACHE_KEY = String.format("%s:%s:%s:%s", dto.getProvince(), dto.getName(), dto.getCode(), dto.getPhonenumber());
